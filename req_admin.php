 <div id="content">
      <br><legend>Manage Requests Data:- <?php if ($_GET['flag'] == 'P') echo "Pending Requests"; else if($_GET['flag'] == 'A') echo "Aprroved Requests"; else echo "Declined Requests" ; ?></legend> 
...
if(isset($_GET['alert'])){
	...
	*/
		...
		if ($flag == 'P')	{
			...
			$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE  status = 'P' ".$sqlfilter;
			...
			$sql="SELECT * FROM requests WHERE  status = 'P' ".$sqlfilter." ORDER BY id LIMIT $start, $limit ";
			...
			$sql="SELECT * FROM requests WHERE  status = 'A' ".$sqlfilter." ORDER BY id LIMIT $start, $limit ";
				$result = $db->query($sql);}
			...
			$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE  status = 'D' ".$sqlfilter;
			...
			$sql="SELECT * FROM requests WHERE  status = 'D' ".$sqlfilter." ORDER BY id LIMIT $start, $limit ";
				$result = $db->query($sql); }
...
<thead>
          <tr>
			...
			<th width="87"> Employee</th>
            <th width="91">Inventory</th>
		...
		<td width="87"><?php echo $row['id']; ?></td>
       	...
       	<td width="87"><?php echo $emp->name; ?></td>

        <td width="91"><?php echo $inv->name; ?></td>
		...
		<td width="130">
		<?php if($flag == 'P' &&  $inv->available > $row['quantity']){
			?>
		<a onclick="return confirm('CONFIRM you want to Allocate  Inventory to this Employee?')" href="index.php?c=req_ans&id=<?php echo $row['id'];?>&op=A">[Approve]</a>
		<a onclick="return confirm('CONFIRM you want to Decline Request?')" href="index.php?c=req_ans&id=<?php echo $row['id'];?>&flag=D">[Decline]</a>
		<?php } 

		else if($flag == 'P' &&  $inv->available < $row['quantity']){
			if ($inv->available>0){
				?>
				<a onclick="return confirm('CONFIRM you want to Allocate Remaining Inventory to this Employee?')" href="index.php?c=req_ans&id=<?php echo $row['id'];?>&op=A&qm=Y"><font color="blue">[Allocate Available]</font></a>
				<a onclick="return confirm('CONFIRM you want to Decline Request?')" href="index.php?c=req_ans&id=<?php echo $row['id'];?>&flag=D">[Decline]</a>
				<?php
