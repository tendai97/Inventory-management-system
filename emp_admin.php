 <div id="content">
      <br><legend>Manage Employee Data:- <?php if ($_GET['flag'] == 'A') echo "Active Employees"; else echo "Inactive Employees"; ?></legend> 
...
if(isset($_GET['alert'])){
	...
	*/
		...
		if ($flag == 'D')	{
			...
			{
			$sqlfilter = " AND name LIKE '%".trim($_POST['searchtxt'])."%' OR surname LIKE '%".trim($_POST['searchtxt'])."%' OR id LIKE '%".trim($_POST['searchtxt'])."%' ";
			}
			...
			$sql="SELECT * FROM users WHERE  active = '0' ".$sqlfilter." ORDER BY name LIMIT $start, $limit ";
			...
			$sql="SELECT * FROM users WHERE  active = '1' ".$sqlfilter." ORDER BY name LIMIT $start, $limit ";
...
<thead>
		...
		<td width="87"><a href="index.php?c=employee_details&id=<?php echo  $row['id'];?>&flag=<?php echo $flag; ?>"><?php echo $row['id']; ?></td>

       	<td width="87"><?php echo $row['name']; ?></td>

        <td width="91"><?php echo $row['surname']; ?></td>

        <td width="117"><?php echo $row['type']; ?></td>

        <td width="117"><?php echo $row['status']; ?></i></a> </td>

		<td width="130">
		<?php if($flag == 'A'){
			?>
		<a href="index.php?c=emp_edit&id=<?php echo $row['id'];?>&op=E">[Edit]</a>
		<a onclick="return confirm('CONFIRM you want to De-activate this Employee Record?')" href="index.php?c=emp_acc&id=<?php echo $row['id'];?>&flag=A">[Deactivate]</a>
		<?php } 
		
		if ($flag == 'D') {
			?>
			<a href="index.php?c=emp_edit&id=<?php echo $row['id'];?>&op=E">[Edit]</a>
			<a onclick="return confirm('CONFIRM you want to Re-Activate this Employee Record?')" href="index.php?c=emp_acc&id=<?php echo $row['id'];?>&flag=D">[Reactivate]</a>
...
<div id="respond"></div>
			...
			<p><a href="index.php?c=emp_edit" class="btn btn-theme-dark btn-ico btn-lg">Add Employee Record <i class="fa fa-user"></i></a></p>
