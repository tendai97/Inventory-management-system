 <div id="content">
      <legend>Trade Lines Data</legend>
...
$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE  type = 'trdlines' and active =1 ".$sqlfilter;
	...
	$targetpage = "index.php?c=trade_admin"; 	//your file name  (the name of this file)
...
$sql="SELECT * FROM $tbl_name WHERE type = 'trdlines' and active= 1".$sqlfilter." ORDER BY name LIMIT $start, $limit ";
...
<thead>
  	...
  	<tr>
		...
		<td width="35"><a href="index.php?c=trade_edit&id=<?php echo $row['id'];?>">[Edit]</a>
		<?php if($row['alert_flag'] =='A' && $row['status']=='New - Awaiting Activation') {?>
		<a onclick="return confirm('CONFIRM you want to Activate this New Trade Line?')" href="index.php?c=trade_edit&id=<?php echo $row['id'];?>&op=A">[Activate]</a>
		<?php } ?>
		<a onclick="return confirm('CONFIRM you want to De-activate this Trade Line?')" href="index.php?c=trade_edit&id=<?php echo $row['id'];?>&op=D">]Deactivate]</a></td>
...
<div id="respond"></div>
<p><a href="index.php?c=trade_edit" class="btn btn-theme-dark btn-ico btn-lg">Add Trade Line <i class="fa fa-user"></i></a></p>
    </div>